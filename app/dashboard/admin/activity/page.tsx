...
const categoryLabels: Record<AdminActivitySummaryKey, string> = { profilePhotos: "Profile photos", profileUpdates: "Profile updates", cityUpdates: "City updates", mapPlaces: "Map places", recommendations: "Recommendations", properties: "Properties" };
...
export default async function AdminActivityPage({ searchParams }: PageProps) {
  ...
  return <DashboardShell eyebrow="Admin" title={selectedCategory ? categoryLabels[selectedCategory] : "Activity"} intro={selectedCategory ? `Showing only ${categoryLabels[selectedCategory].toLowerCase()} activity.` : "A human-readable history of provider, city, property, and approval work across the platform."}>
